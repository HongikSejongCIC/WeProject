    ...
    public static void start(String data) {
        ...
        try {
            ...
            if(data.substring(0,1).equals("1") && data.length() == 3 ){
                URL url = new URL("http://192.168.1.5/test/Data_insert36.php?data="+data.substring(1,3)); // 호출할 url
                url.openStream();
            }else if(data.substring(0,1).equals("2") && data.length() == 3){
                URL url = new URL("http://192.168.1.5/test/Data_insert37.php?data="+data.substring(1,3)); // 호출할 url
                url.openStream();
            }
